...
Shared constants for the P&ID symbol library:
  - schema version
  - domain types (SymbolId, Standard, Category, etc.)
...
Standard = NewType("Standard", str)
Category = NewType("Category", str)
Subcategory = NewType("Subcategory", str)
Confidence = NewType("Confidence", str)
ClassificationMethod = NewType("ClassificationMethod", str)
...
_REFERENCE_RE = re.compile("|".join(_REFERENCE_SHEET_PATTERNS), re.IGNORECASE)

# Pattern to extract standard from filename: "(ISO 10628-2)" or "(DIN 2429)"
_STANDARD_RE = re.compile(
    r"\(\s*((?:ISO|DIN|ISA)\s*[\d\-]+(?:-\d+)?)\s*\)", re.IGNORECASE
)
...
_BUBBLE_CATS: frozenset[str] = frozenset(
    {"instrument_bubble", "instrument", "annotation"}
)
...
_MINIFY_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (
        re.compile(r"\s*<(sodipodi|inkscape):[^\s>][^>]*>.*?</\1:[^>]*>\s*", re.DOTALL),
        "",
    ),
...
# Degradation / Augmentation constants

# Pixel value boundaries
WHITE_THRESHOLD: int = 250
INK_THRESHOLD: int = 128
GRAY_VALUE_THRESHOLD: int = 230

# Paper texture defaults
PAPER_BASE_COLOR: tuple[int, int, int] = (248, 243, 232)
PAPER_MIN_PIXEL_VALUE: int = 200
PAPER_MAX_PIXEL_VALUE: int = 255

# Age-related color tints
CREAM_TINT: tuple[int, int, int] = (245, 235, 200)
FADED_PAPER_TONE: tuple[int, int, int] = (218, 212, 200)

# Noise and distortion intensity defaults
NOISE_SIGMA_MAX: float = 30.0
JPEG_QUALITY_MIN: int = 5
JPEG_QUALITY_MAX: int = 80

# Bounding box padding
TIGHT_BBOX_PADDING: int = 1

# Canvas defaults for composition
DEFAULT_CANVAS_SIZE: int = 640
MIN_SYMBOL_FRACTION: float = 0.12
MAX_SYMBOL_FRACTION: float = 0.32

# IoU threshold for symbol placement

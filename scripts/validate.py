...
def _check(
    errors: list, warnings: list, path: str, condition: bool, message: str
) -> None:
    if not condition:
...
def validate_symbol(data: dict, json_path: Path) -> tuple[list[str], list[str]]:
    ...
    for field in REQUIRED_FIELDS:
        _check(
            errors, warnings, name, field in data, f"missing required field '{field}'"
        )
    ...
    _check(
        ...
        warnings,
        name,
        bool(isinstance(sv, str) and re.match(r"^\d+\.\d+\.\d+$", sv)),
    ...
    _check(
        ...
        warnings,
        name,
        isinstance(sym_id, str) and sym_id.count("/") >= 2,
    ...
    _check(
        ...
        warnings,
        name,
        isinstance(fn, str) and fn.endswith(".svg"),
    ...
    _check(
        ...
        warnings,
        name,
        isinstance(fn, str) and fn == fn.lower(),
    ...
    _check(
        ...
        warnings,
        name,
        isinstance(dn, str) and len(dn) >= 1,
    ...
    _check(
        ...
        warnings,
        name,
        isinstance(cat, str) and len(cat) >= 1,
        "category must be a non-empty string",
    )
    _check(errors, warnings, name, isinstance(sub, str), "subcategory must be a string")
    if cat == "unknown":
    ...
    if isinstance(svg, dict):
        ...
        if isinstance(ec, int) and ec == 0:
            _warn(
                warnings,
                name,
                "svg.element_count is 0 — SVG may be empty or unparseable",
            )
        _check(
            errors,
            warnings,
            name,
            isinstance(svg.get("has_text"), bool),
        ...
        if svg.get("view_box") is None:
            _warn(
                warnings,
                name,
                "svg.view_box is null — symbol lacks a viewBox attribute",
            )
    else:
    ...
    # tags
    tags = data.get("tags", [])
    _check(errors, warnings, name, isinstance(tags, list), "tags must be an array")
    ...
    if not expected_svg.exists():
        errors.append(
            f"  ERROR  {name}: companion SVG '{expected_svg.name}' not found on disk"
        )
...
def main() -> None:
    ...
    if not schema_path.exists():
        print(
            f"WARNING: schema file not found: {schema_path} — schema version check skipped."
        )
        _schema = {}
    else:
    ...
    json_files = sorted(
        f for f in processed_dir.rglob("*.json") if f.name != "registry.json"
    )
